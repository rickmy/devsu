...
export class NewComponent implements OnDestroy {
  ...
  constructor(
    ...
    this.f['date_release'].valueChanges.pipe(
      distinctUntilChanged(),
      takeUntil(this.unsubscribe$)
  ...
  verifyId(id: string) {
    this._productsService.verifyId(id).pipe(
      takeUntil(this.unsubscribe$)
    ).subscribe((response) => {
      return response ? this.f['id'].setErrors({ 'idExists': true }) : this.f['id'].setErrors(null);
    });
  ...
  save() {
    ...
    if (!this.product) this.createProduct(); else this.updateProduct();
  }
